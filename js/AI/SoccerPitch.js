...
class SoccerPitch {
    ...
    constructor( scene3D ) {
        ...
        this.m_pBall = new SoccerBall( this.scene3D );
...
class SoccerBall extends PlayerBase {
    ...
    Render() {
        ...
        const ballPos = this.scene3D.convertRange( this.m_vPosition );
        this.scene3D.ball3D.position.set( ballPos.x, 5, ballPos.y );
...
class FieldPlayer extends PlayerBase {
    ...
    constructor( home_team ) {
        ...
        super( home_team );
        this.scene3D = home_team.scene3D;
        this.index = this.scene3D.playerIndex;
    ...
    Render() {
        ...
        this.scene3D.players[ this.index ].position.set( playerPos.x, 0, playerPos.y );

        //Ring glow & heatmap ( player selected )
        if ( this.index == this.scene3D.playerSelected ){
            this.scene3D.ring.position.set( playerPos.x, 0, playerPos.y );
        };
        ...
        //Player Rotation
		this.scene3D.players[ this.index ].rotation.y = angleRotation * -1;
		...
		// animate player
		if( speed == 0 ) {  //idle
			this.scene3D.mixer[ this.index ].clipAction( 'idle' ).setEffectiveWeight( 1 );    				
			this.scene3D.mixer[ this.index ].clipAction( 'walk' ).setEffectiveWeight( 0 );
			this.scene3D.mixer[ this.index ].clipAction( 'run' ).setEffectiveWeight( 0 );
		};

		if( speed == 1 ) {  //run
			this.scene3D.mixer[ this.index ].clipAction( 'idle' ).setEffectiveWeight( 0 );    				
			this.scene3D.mixer[ this.index ].clipAction( 'walk' ).setEffectiveWeight( 0 );
			this.scene3D.mixer[ this.index ].clipAction( 'run' ).setEffectiveWeight( 1 );
		};

		if( speed > 0 && speed <= 0.5 ) { // from idle to walk < - > from walk to idle
			this.scene3D.mixer[ this.index ].clipAction( 'idle' ).setEffectiveWeight( 1 - ( speed / 0.5 ) );    				
			this.scene3D.mixer[ this.index ].clipAction( 'walk' ).setEffectiveWeight( speed / 0.5  );
			this.scene3D.mixer[ this.index ].clipAction( 'run' ).setEffectiveWeight( 0 );
		};

		if( speed > 0.5 && speed < 1 ) { // from walk to run < - > from run to walk
			this.scene3D.mixer[ this.index ].clipAction( 'idle' ).setEffectiveWeight( 0 );    				
			this.scene3D.mixer[ this.index ].clipAction( 'walk' ).setEffectiveWeight( 1 - ( ( speed - 0.5 ) / 0.5 ) );
			this.scene3D.mixer[ this.index ].clipAction( 'run' ).setEffectiveWeight( ( speed - 0.5 ) / 0.5 );
		};
...
class GoalKeeper extends PlayerBase {
    ...
    Render() {
        ...
        const playerPos = this.scene3D.convertRange( this.Pos() );
        this.scene3D.players[ this.index ].position.set( playerPos.x, 0, playerPos.y );
